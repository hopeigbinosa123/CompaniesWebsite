...
class SoftwareService(models.Model):
    ...
    estimated_timeline = models.CharField(
        max_length=100, default="default timeline"
    )  # e.g., "2-4 weeks"
...
class ProjectUpdate(models.Model):
    service_request = models.ForeignKey(
        ServiceRequest, on_delete=models.CASCADE, related_name="updates"
    )
    title = models.CharField(max_length=200)
    description = models.TextField()
    created_at = models.DateTimeField(auto_now_add=True)
...
class SupportResponse(models.Model):
    enquiry = models.ForeignKey(
        SoftwareEnquiry, on_delete=models.CASCADE, related_name="responses"
    )
