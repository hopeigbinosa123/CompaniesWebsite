...
class CreatePayPalOrderView(APIView):
    def post(self, request):
        ...
        description = serializer.validated_data.get('description', 'Order from OSIJ Platform')
        try:
            payment = Payment({
                "intent": "sale",
                "payer": {
...
class CapturePayPalOrderView(APIView):
    def post(self, request, order_id):
        try:
            ...
            paypal_payment = Payment.find(payment_id) # Renamed to avoid conflict with local Payment model
            ...
            if paypal_payment.execute({"payer_id": payer_id}):
                ...
                return Response({"message": "Payment captured successfully!", "payment_id": payment_record.id}, status=status.HTTP_200_OK)
            else:
                # If PayPal execution fails, update payment status to FAILED if a record exists
                payment_record = PaymentRecord.objects.filter(paypalOrder_id=order_id).first()
                if payment_record:
            ...
            import traceback
            traceback.print_exc()
            return Response({"error": "An unexpected error occurred during payment capture.", "detail": str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)