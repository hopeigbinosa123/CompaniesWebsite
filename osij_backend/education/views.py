...
class CourseLessonsView(generics.ListAPIView):
    ...
    def get_queryset(self):
        return Lesson.objects.filter(course_id=self.kwargs["pk"], is_active=True)
    ...
    def post(self, request, lesson_id):
        try:
            ...
            if not created:
                ...
                progress.time_spent_minutes = request.data.get(
...
class UpcomingLiveSessionsView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        return LiveSession.objects.filter(start_time__gte=timezone.now()).order_by(
            "start_time"
        )
